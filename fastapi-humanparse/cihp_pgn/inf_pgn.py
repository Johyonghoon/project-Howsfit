...
from cihp_pgn.utils.image_reade_inf import *
from cihp_pgn.utils.utils import *
from cihp_pgn.utils.model_pgn import *
...
def get_humanparse(model_fname):
    start = time.time()

    for i in glob(os.path.join(f'data/image/{model_fname}'), recursive=True):
        if os.path.isfile(i):
    ...
    parse_image = './output/parse_image'
    print(f" >>>>> 사용 준비 시간 : {round((time.time() - start), 2)}")

    # Iterate over training steps.
    for step in range(NUM_STEPS):
        start = time.time()
        parsing_, scores = sess.run([pred_all, pred_scores])  # , edge_ / , pred_edge
        img_split = image_list[step].split('/')
        img_name, img_ext = os.path.splitext(img_split[-1])
        fname = f"{parse_image}/{img_name}_color.png"
        msk = decode_labels(parsing_, num_classes=N_CLASSES)

        parsing_im = Image.fromarray(msk[0])
        parsing_im.save(fname)
        cv2.imwrite(f'{parsing_dir}/{img_name}.png', parsing_[0, :, :, 0])
        # os.remove(image_list[step])
        print(f" >>>>> 이미지 처리시간 : {round((time.time() - start), 2)}초")
