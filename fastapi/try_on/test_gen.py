...
import os
import time
from cp_test import CPDatasetTest, CPDataLoader
...
def get_opt():
    ...
    parser.add_argument("--output_dir", type=str, default="./viton_output")
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            if opt.cuda:
                ...
                warped_cm_onehot = torch.FloatTensor(
                    (warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(np.float))
